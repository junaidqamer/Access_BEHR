...
contract patientVisit {
    ...
    //All charges paid/ Visit/Contract closed.
    event visitClosed(string condition);
...
// Demographics will always be first thing reviewed. 
  ...
  function addPatientQuestionnaire(
    ...
    //Notes will have to be reffered to by address and name, example: ReferralAddress(CompanyName): Notes; ReferallAddress2(CompanyName): Notes; etc.
    ...
    //Medication
    // Medications will be represented here as addresses or uint (have to decide).
    function orderMedication(address[] memory _medicationOrders, string memory _medicationNotes) public{
        medicationList = _medicationOrders;
        medicationNotes = _medicationNotes;
        emit medicationOrdered('Medications have been ordered. Please request the Medication Notes for further details.');
    }
    
    function getOrderedMedications() view public returns(address[] memory){
        return medicationList;
    }
    
    //Notes will have to be reffered to by address and name, example: RxAddress(Name): Notes; RxAddress2(Name): Notes; etc.
    function getOrderedMedicationsNotes() view public returns(string memory){
        return medicationNotes;
    }
    // Would like to do something sending medication requests to pharmacy. But we are currently in infancy.
    
    //VISIT SUMMARY
    //Here the provider will be able to put all of there notes for the whole visit. This will be a general summary.
    //The provider can write do the medications ordered, labs ordered, referrals, a follow-up and etc. This will
    // be EVERYTHING basically found above in a quick summary.
    function visitsSummary(string memory _visitsSummary) public {
        visitSummary = _visitsSummary;
    }
    
    function getVisitSummary() view public returns(string memory){
        return visitSummary;
    }
    
    function requestFollowUp() public{
        emit followUpNeeded('Please schedule a follow up at your earliest convenience relating to the prescribed health plan.');
    }
    
    //PAYMENT
    //We are currently using a self-pay model but would like to eventually develop a minting process with insurance. 
    //In our current self-pay model. The provider will request ethereum once this contract is deployed.
    //The patient is the receiver of the contract so the contract is deployed and the request goes immediately to the recipients wallet.
   
    mapping(address => uint) balances;
    
    //Get patient wallet information.
    function accessPatientWallet(address payable _patientWallet) public {
        patientWallet = _patientWallet;
    }
    
    //Check patients wallet balance.
    function balance() public view returns(uint) {
        return balances[patientWallet];   // Questions: Why is my balances not working.?
    }
    
    //Get amount for payment.
    function setBilling(uint amount) public {
        charges = amount;
    }
    
    //See how much the visit was.
    function getCharges() view public returns(uint){
        return charges;
    }
    
    //Take payment.
    function completeBilling() public {
        balances[patientWallet] = balances[patientWallet].sub(charges); 
        balances[provider] = balances[provider].add(charges);
        emit visitClosed('Everything has been charged and the billing has been completed. This visit is now closed.');
    }
